...
resource "aws_s3_bucket" "terraform_state" {
  bucket = "cv-challenge04-terraform-state-backend" # Replace with your unique bucket name
...
resource "aws_dynamodb_table" "terraform_locks" {
  name         = "cv-challenge04-terraform-locks"
...
data "aws_iam_policy_document" "combined_backend_policy" {
  statement {
    ...
    resources = [
      "arn:aws:s3:::cv-challenge04-terraform-state-backend",
      "arn:aws:s3:::cv-challenge04-terraform-state-backend/*"
    ]
  }
  ...
  statement {
    ...
    resources = [
      "arn:aws:dynamodb:us-east-1:931058976119:table/cv-challenge04-terraform-locks" # Replace with your AWS Account ID
    ]
  }
}
